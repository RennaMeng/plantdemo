...
export function PlantPreview({ selectedFeatures, compact = false }: PlantPreviewProps) {
    ...
    <section className={`rounded-lg border border-archive-line bg-archive-label shadow-specimen ${compact ? "p-3" : "p-5"}`}>
      {!compact && (
        <div className="mb-4 flex items-start justify-between gap-4 border-b border-archive-line pb-3">
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-archive-moss">Botanical Illustration Preview</p>
            <h2 className="mt-1 font-serif text-2xl text-archive-ink">植物科学插画预览</h2>
          </div>
          <span className="rounded border border-archive-line bg-white/70 px-2 py-1 text-xs text-archive-ink/70">Mock SVG</span>
        </div>
      )}
